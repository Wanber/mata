...
<div class="wrap">
    ...
    <table class="wp-list-table widefat fixed striped posts">
        ...
        <script>
            function deletar(id, nome) {
                if(confirm("Deseja realmente deletar '" + nome + "' ?"))
                    location.href = "admin.php?page=<?php echo PLUGIN_PREFIX ?>pendentes&excluir=" + id;
            }
        </script>
        ...
        <?php
        global $wpdb;
        $registros = $wpdb->get_results( "SELECT * FROM ".$wpdb->prefix.PLUGIN_PREFIX."dados"." WHERE modalidade = 'empreendimento' AND aprovado = '0'" );
        if($wpdb->num_rows > 0)
            foreach ($registros as $registro) {
                ...
                <tr id="post-1" class="iedit author-self level-0 post-1 type-post status-publish format-standard hentry category-sem-categoria">
                    ...
                    if($registro->aprovado == '0')
                        echo '<a style="color: #39668f" href="admin.php?page='.PLUGIN_PREFIX.'novo&editar='.$registro->id.'">REVISAR</a>&nbsp;&nbsp';
                    ?>
                ...
                <tr id="post-1" class="iedit author-self level-0 post-1 type-post status-publish format-standard hentry category-sem-categoria">
                        <td class="title column-title has-row-actions column-primary page-title" data-colname="Título">
                            <strong><a class="row-title" href="#">Nenhum Parceiro encontrado</a></strong>
                        </td>
