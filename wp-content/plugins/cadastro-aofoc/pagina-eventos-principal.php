<?php function pagina_eventos_principal() { ?>
<div class="wrap">
    <h1>Eventos <a href="admin.php?page=<?php echo PLUGIN_PREFIX ?>eventos_novo" class="page-title-action">Adicionar Novo</a></h1>
    <br />
    ...
    <table class="wp-list-table widefat fixed striped posts">
        ...
        <script>
            function deletar(id, nome) {
                if(confirm("Deseja realmente deletar '" + nome + "' ?"))
                    location.href = "admin.php?page=<?php echo PLUGIN_PREFIX ?>eventos&excluir=" + id;
            }
        </script>
        ...
        <?php
        global $wpdb;
        $registros = $wpdb->get_results( "SELECT * FROM ".$wpdb->prefix.PLUGIN_PREFIX."dados"." WHERE modalidade = 'evento' AND aprovado = '1'" );
        if($wpdb->num_rows > 0)
            foreach ($registros as $registro) { ?>
                <tr id="post-1" class="iedit author-self level-0 post-1 type-post status-publish format-standard hentry category-sem-categoria">
                    ...
                    <td class='modalidade column-modalidade'"><?php echo $registro->endereco ?></td>
                    <td class='tipo column-tipo'"><?php echo $registro->dt_inicio.'-'.$registro->dt_fim ?></td>
                    <td class='categories column-categories'">
                        <a style="color: #3e8f3e" href="admin.php?page=<?php echo PLUGIN_PREFIX ?>eventos_novo&editar=<?php echo $registro->id ?>">EDITAR</a>&nbsp;&nbsp;
                        <a style="color: #e73746" href="javascript: deletar(<?php echo $registro->id ?>,'')">EXCLUIR</a>&nbsp;&nbsp;
                        <?php
                        if($registro->aprovado == '0')
                            echo '<a style="color: #39668f" href="admin.php?page='.PLUGIN_PREFIX.'eventos_novo&editar='.$registro->id.'">ACEITAR</a>';
                        ?>
                    </td>
                </tr>
                <?php
