...
<div id="huge_it_slider">
  <?php 
	  ...
	  $id=intval($_POST["hugeit_slider_id"]);
	  }
	  else{
	  $id=($firstrow->id);
	  }
	 if(isset($_GET["htslider_id"]) AND  (intval($_GET["htslider_id"]) == intval($_POST["hugeit_slider_id"]))){
		 if(isset($_GET["hugeit_save"])) {
			 $hugeit_save = intval($_GET["hugeit_save"]);
              if($hugeit_save ==1 ){
