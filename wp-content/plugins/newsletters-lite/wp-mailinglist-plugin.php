	...
	class wpMailPlugin extends wpMailCheckinit {
	    ...
	    function ajax_importfile() {
			...
			if ( $movefile && ! isset( $movefile['error'] ) ) {
			    ...
			    $csvtypes = array('text/comma-separated-values', 'text/csv', 'application/csv', 'application/excel', 'application/vnd.ms-excel', 'application/vnd.msexcel', 'text/anytext', 'text/plain');
	    ...
	    function ajax_welcomestats() {
			...
			switch ($type) {
				case 'years'			:
				    ...
				    // Reads
				    $query = "SELECT COUNT(id) as `readscount`, DATE(created) as `date` FROM " . $wpdb -> prefix . $Email -> table . " WHERE" . $history_condition . " `read` = 'Y' AND CAST(`created` AS DATE) BETWEEN '" . $fromdate . "' AND '" . $todate . "' GROUP BY DATE(`created`)";
				    $records = $wpdb -> get_results($query);
		...
		function paidsubscription_form($subscriber = null, $mailinglist = null, $autosubmit = true, $target = "_self", $extend = false) {
			global $Html;
		...
		function sc_management($atts = array(), $content = null) {
			global $wpdb, $Db, $Subscriber, $SubscribersList, $Auth, $Unsubscribe, 
			$newsletters_errors, $Field;
			...
			switch ($method) {
				...
				case 'unsubscribe'			:
					...
					$this -> render('unsubscribe-success' . $userfile, array('subscriber' => $subscriber, 'dorender' => $dorender, 'user' => $user, 'data' => $data, 'errors' => $errors, 'success' => $success, 'deleted' => $deleted), true, 'default');
					break;
				case 'logout'				:
				...
				if ($field = $Field -> get($field_id)) {
					...
					switch ($field -> type) {
						case 'hidden'			:						
							...
							break;
						case 'text'				:						
							echo '<input' . $placeholder . ' class="form-control ' . $this -> pre . ' ' . $this -> pre . 'text' . ((!empty($Subscriber -> errors[$field -> slug])) ? ' newsletters_fielderror' : '') . '" id="' . $this -> pre . '-' . $optinid . '' . $field -> slug . '" ' . $Html -> tabindex($optinid) . ' type="text" name="' . $field -> slug . '" value="' . esc_attr(stripslashes($Subscriber -> data[$field -> slug])) . '" />';
							break;
						case 'special'			:						
							switch ($field -> slug) {
								case 'list'				:
										...
										if (!empty($list)) {																						
											switch ($list) {
												...
												case 'checkboxes'		:
													...
													if (!empty($lists)) {
														//echo '<div class="checkbox"><label style="font-weight:bold;"><input onclick="jqCheckAll(this, false, \'list_id\');" type="checkbox" name="checkboxall" value="1" id="checkboxall" /> ' . __('Select all', $this -> plugin_name) . '</label></div>';
														foreach ($lists as $list_id => $list_title) {
							...
							break;
						case 'textarea'			:
							echo '<textarea' . $placeholder . ' class="form-control ' . $this -> pre . ' ' . $this -> pre . 'textarea' . ((!empty($Subscriber -> errors[$field -> slug])) ? ' newsletters_fielderror' : '') . '" id="' . $this -> pre . '-' . $optinid . '' . $field -> slug . '" ' . $Html -> tabindex($optinid) . ' rows="4" name="' . $field -> slug . '">' . strip_tags($Subscriber -> data[$field -> slug]) . '</textarea>';
							break;
						case 'select'			:						
							echo '<select' . $placeholder . ' class="form-control ' . $this -> pre . ' ' . $this -> pre . 'select' . ((!empty($Subscriber -> errors[$field -> slug])) ? ' newsletters_fielderror' : '') . '" style="width:auto;" id="' . $this -> pre . '-' . $optinid . '' . $field -> slug . '" ' . $Html -> tabindex($optinid) . ' name="' . $field -> slug . '">';
							echo '<option value="">' . __('- Select -', $this -> plugin_name) . '</option>';
						...
						case 'radio'			:							
							if (!empty($field -> newfieldoptions)) {
								foreach ($field -> newfieldoptions as $option_id => $option_value) {																		
									...
									echo '<input class="' . $this -> pre . 'radio' . ((!empty($Subscriber -> errors[$field -> slug])) ? ' newsletters_fielderror' : '') . '" ' . $Html -> tabindex($optinid) . ' type="radio" ' . $checked . ' name="' . $field -> slug . '" value="' . $option_id . '" /> ' . __($option_value);
									echo '</label>';
									echo '</div>';
								}
							}
						...
						case 'checkbox'			:							
							if (!empty($field -> newfieldoptions)) {
								foreach ($field -> newfieldoptions as $option_id => $option_value) {
									$checked = (!empty($Subscriber -> data[$field -> slug]) && (is_array($Subscriber -> data[$field -> slug]) && in_array($option_id, $Subscriber -> data[$field -> slug]))) ? 'checked="checked"' : '';
									echo '<div class="checkbox">';
									echo '<label class="control-label wpmlcheckboxlabel ' . $this -> pre . '">';
									echo '<input class="' . $this -> pre . 'checkbox' . ((!empty($Subscriber -> errors[$field -> slug])) ? ' newsletters_fielderror' : '') . '" ' . $Html -> tabindex($optinid) . ' type="checkbox" ' . $checked . ' name="' . $field -> slug . '[]" value="' . $option_id . '" /> ' . __($option_value);
									echo '</label>';
									echo '</div>';
								}
							}
						...
						case 'file'				:						
							...
							<button class="btn btn-default btn-file">
								<?php _e('Browse...', $this -> plugin_name); ?> <input onchange="jQuery('#file_upload_<?php echo $field -> id; ?>_info').html(jQuery(this).val());" type="file" <?php echo $Html -> tabindex($optinid); ?> name="<?php echo esc_attr(stripslashes($field -> slug)); ?>" id="file_upload_<?php echo $field -> id; ?>" />
							</button>
						...
						case 'pre_country'		:
							...
							if ($countries = $this -> Country() -> select()) {						
								...
								<select class="<?php echo $this -> pre; ?>country <?php echo $this -> pre; ?> <?php echo $this -> pre; ?>precountry<?php echo ((!empty($Subscriber -> errors[$field -> slug])) ? ' newsletters_fielderror' : ''); ?>" id="<?php echo $this -> pre; ?>-<?php echo $optinid . $field -> slug; ?>" <?php echo $Html -> tabindex($optinid); ?> name="<?php echo $field -> slug; ?>">
						...
						case 'pre_date'			:
							...
							<input type="text" class="<?php echo $this -> pre; ?>predate <?php echo $this -> pre; ?>text <?php echo $this -> pre; ?> <?php echo $this -> pre; ?>predate<?php echo ((!empty($Subscriber -> errors[$field -> slug])) ? ' newsletters_fielderror' : ''); ?>" value="<?php echo $currentDate; ?>" name="<?php echo $field -> slug; ?>" id="<?php echo $this -> pre; ?>-<?php echo $optinid . $field -> slug; ?>" />
						...
						case 'pre_gender'		:
							...
							<select<?php echo $placeholder; ?> class="<?php echo $this -> pre; ?> <?php echo $this -> pre; ?>pregender<?php echo ((!empty($Subscriber -> errors[$field -> slug])) ? ' newsletters_fielderror' : ''); ?>" style="width:auto;" id="<?php echo $this -> pre; ?>-<?php echo $optinid; ?><?php echo $field -> slug; ?>" <?php echo $Html -> tabindex($optinid); ?> name="<?php echo $field -> slug; ?>">
		...
		function gen_tracking_link($eunique = null) {
			...
			if (!empty($eunique)) {
				if ($this -> get_option('tracking') == "Y") {
					...
					$tracking = '<img class="newsletters-tracking" src="' . esc_url(add_query_arg(array($this -> pre . 'method' => "track", 'id' => $eunique), home_url())) . '" />';
				}	
			}
		...
		function updating_plugin() {
			...
			if (version_compare($this -> version, $cur_version) === 1) {				
				...
				if (version_compare($cur_version, "4.6.2") < 0) { 
					$this -> update_options();
					
					$this -> update_option('showmessage_newsystememails', true);
			...
			if (!empty($link)) {			
				if (!preg_match("/(manage\-subscriptions|loginauth|wpml|wpmlmethod|jpg|png|gif|jpeg|bmp|wpmltrack|wpmllink)/si", $link)) {
					if (preg_match("/^http\:\/\//si", $link) || preg_match("/^https\:\/\//si", $link)) {					
						...
						$hashlink = esc_url(add_query_arg($queryargs, home_url()));
