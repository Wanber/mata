	...
	class wpMail extends wpMailPlugin {
		...
		function admin_notices() {
			...
			if (is_admin()) {
				...
				$showmessage_newsystememails = $this -> get_option('showmessage_newsystememails');
				if (!empty($showmessage_newsystememails)) {
					$message = sprintf(__('Newsletters: Some changes to "Posts" and "Latest Posts" system emails. Compare %s to %s', $this -> plugin_name), '<a href="' . admin_url('admin.php?page=' . $this -> sections -> settings_templates) . '">' . __('yours', $this -> plugin_name) . '</a>', '<a href="http://tribulant.com/docs/wordpress-mailing-list-plugin/6104" target="_blank">' . __('the latest ones', $this -> plugin_name) . '</a>');
					$message .= ' <a href="' . $Html -> retainquery('newsletters_method=hidemessage&message=newsystememails') . '"><i class="fa fa-times"></i> ' . __('Hide', $this -> plugin_name) . '</a>';
					$this -> render_message($message);
				}
						...
						if (!empty($_GET['message'])) {
							switch ($_GET['message']) {
								...
								case 'queue_status'				:
									...
									break;
								case 'newsystememails'			:
									$this -> delete_option('showmessage_newsystememails');
									break;
							}
						}
			...
			switch ($method) {
				...
				default 						:
					...
					if (!empty($_GET['showall'])) {
						...
						$data = $this -> paginate($this -> Template() -> model, null, $this -> sections -> templates, $conditions, $searchterm, $perpage, $order);
					}
			...
			/*if ($plugin == plugin_basename(__FILE__)) {				
				...
				if (!empty($old_folder)) {					
					...
					if (!empty($old_folder_name)) {						
						$this -> update_option('old_folder_name', $old_folder_name);
											
						if ($this -> recurse_copy($old_folder, $new_folder)) {											
