		...
		function shortcode_posts($atts = array(), $content = null, $tag = null) {
			...
			switch ($tag) {
				...
				case 'newsletters_post_loop'	:				
					...
					$emptyagain = false;
					if (empty($shortcode_posts) && !empty($shortcode_sendas)) {
						$shortcode_posts = $shortcode_sendas;
						$emptyagain = true;
					}
					...
					if (!empty($emptyagain) && $emptyagain == true) {
						$shortcode_posts = false;
					}
				...
				case 'newsletters_post_anchor'	:
					if (!empty($shortcode_post)) {
						return '<a title="' . esc_attr(stripslashes(__($shortcode_post -> post_title))) . '" href="' . $this -> direct_post_permalink($shortcode_post -> ID) . '">' . __($shortcode_post -> post_title) . '</a>';
					}
					break;
				case 'post_title'				:
				...
				case 'newsletters_post_link'	:
					if (!empty($shortcode_post)) {						
						return $this -> direct_post_permalink(__($shortcode_post -> ID));
					}
					break;
					...
					if (!empty($shortcode_post)) {
						if (function_exists('has_post_thumbnail')) {
							if (has_post_thumbnail($shortcode_post -> ID)) {							
								...
								$return .= '</a>';						
								return apply_filters('newsletters_post_thumbnail_output', $return, $shortcode_post);
							}
						}
					}
					break;
