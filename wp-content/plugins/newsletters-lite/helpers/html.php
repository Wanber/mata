...
class wpmlHtmlHelper extends wpMailPlugin {
	...
	function help($help = null, $content = null) {
		if (!empty($help)) {
			ob_start();
			...
			<span class="wpmlhelp"><a href="" onclick="return false;" title="<?php echo esc_attr(stripslashes($help)); ?>"><?php echo stripslashes($content); ?></a></span>
	...
	function eunique($subscriber = null, $email_id = null) {
		if (!empty($subscriber) && !empty($email_id)) {
			return md5($subscriber -> id . $subscriber -> mailinglist_id . $email_id . date_i18n("YmdH"));	
		}
	}
